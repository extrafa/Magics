...
**Проверено.** Живьём в симуляторе подтверждены все ветки: показ на 3-м
закрытии, дизлайк не пишет ответ сам по себе, любое закрытие шторки (включая
свайп/тап мимо) ставит снюз на 14 дней и блокирует повтор, «Write to us»
ставит постоянный флаг.

## Финальный ревью-проход

`MockRateAppPreferences` в тестах изначально был `struct`. `RateAppViewModel`
хранит СВОЮ копию `preferences` (value type через протокол), поэтому мутации
через VM были не видны в тесте — 3 из 4 тестов реально падали бы. Поменян на
`final class` (reference type — VM и тест видят один и тот же объект), после
чего все 4 теста пройдены по-настоящему (`xcodebuild test`, не только
компиляция).
